...
from experiments.model_factories import CWAEFactory, WAEFactory, SWAEFactory
...
parser.add_argument('batch_size_n', type=int)
parser.add_argument('batch_size_k', type=int)
...
if args.batch_size_k > args.batch_size_n:
    raise ValueError('K cannot be larger than N')

batch_size = args.batch_size_k
extra_latent_size = args.batch_size_n - args.batch_size_k

dataset_loader = get_dataset_loader(args.dataset_name, batch_size)
base_dir = prepare_base_dir(args)
optimizer_factory = get_optimizer_factory(args.optimizer, args.learning_rate)
model_factories = {
    'cwae': lambda: CWAEFactory(),
    'wae': lambda: WAEFactory(),
    'swae': lambda: SWAEFactory()
}
...
experiment_name = f'{model_name}_n{args.batch_size_n}_k{args.batch_size_k}_o{args.optimizer}_lr{args.learning_rate}'
...
with tf.Session(config=tf.ConfigProto(gpu_options=tf.GPUOptions(allow_growth=True))) as tf_session:
    training_process.start(tf_session, [extra_latent_size, args.latent_size])
    ...
    for current_epoch in range(1, epochs_count+1):
        ...
        if (current_epoch == 1 or current_epoch % args.report_after_epoch == 0 or current_epoch == epochs_count):
            ...
            if interpolations_count > 0:
                ...
                print(interpolations_step_count + 4)
                training_process.save_image(f'interpolations.png', images, size=(interpolations_step_count + 4, 
                                                                                 interpolations_count))
